...
def process_batch(
    ...
    for i in range(len(iouv)):
        ...
        if x[0].shape[0]:
            ...
            correct[matches[:,1].astype(int),1] = True
...
#     def __init__(self,nc,conf=0.25,iou_thres=0.45) -> None:
#         self.matrix = np.zeros((nc+1,nc+1))
#         self.nc = nc        #number of classes
#         self.conf = conf
#         self.iou_thres = iou_thres

    # def process_batch(self,dtections,labels):
    #     """
    #         input   dtections[x1 y1 x2 y2 conf class]
    #                 labels[class x1 y1 x2 y2]
    #         output  updates confusion matrix     

    #     """
    #     if dtections is None:
    #         get_classes = labels.int()
    #         for gc in get_classes:
    #             self.matrix[self.nc,gc] += 1    # background FN
    #         return
        
    #     dtections = dtections[dtections[:,4]>self.conf]
    #     gt_classes = labels[:,0].int()
    #     dtections_classes = dtections[:,5].int()
    #     iou = box_iou(labels[:,1:],dtections[:,:4])  # tensor N M
